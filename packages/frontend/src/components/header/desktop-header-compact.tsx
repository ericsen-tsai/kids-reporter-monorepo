...
export function DesktopHeaderCompact({
  ...
  return (
    <div
      className={cn(
        'w-full bg-white hidden desktop:block fixed top-0 left-1/2 transform -translate-x-1/2 z-1000 transition-all duration-300 ease-in-out shadow-lg',
        isAtTop
          ? 'opacity-0 -translate-y-full pointer-events-none'
          : 'opacity-100 translate-y-0 pointer-events-auto'
      )}
    >
      <div className="flex items-center justify-between py-6 desktop:px-16 max-w-300 mx-auto">
        ...
        <div className="flex items-center gap-4">
          <ActionButtons hideCtaButtons={true} />
          <Link
            href="/login"
